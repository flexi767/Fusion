...
<!-- FNXC:FullSuiteReconciliation 2026-09-23-04:51: FN-9369 aligns Full Suite runtime consumers with the PR Gate's mandatory pre-consumer build, so cold or incomplete workspace output cannot create a main-only lane failure. -->
### Full Suite runtime reconciliation

FN-9369 identified the common main-only condition behind the recurring `Pipeline smoke tier` plus four-shard failure family: Full Suite runtime consumers started after installation without a mandatory workspace build, while the green PR Gate restored output and ran `pnpm build` before boot smoke and tests. Exact `dist-*` cache misses and the independent Pipeline smoke lane both exposed that missing reconciliation; changing individual failing tests could not converge because their names churned behind the same lane-level condition.

`test-shards` now computes its source hash after checkout, restores only an exact `dist-*` key, seeds the artifact hash cache only for an exact hit, and then runs the fast-CLI workspace build before its shard command. `test-pipeline-smoke` performs the same fast-CLI build before its independent smoke command. The Full Suite remains push-only and non-blocking, retains SHA-keyed no-cancel concurrency, full Git history for real-Git consumers, PostgreSQL readiness, four shards, and timing artifact uploads. The residual PR differences—pull-request cancellation, shallow Gate checkout, and Gate-only incremental cache namespace—are intentional and are not runtime-consumer reconciliation differences.

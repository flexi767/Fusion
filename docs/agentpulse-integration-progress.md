...
## Lag rollout and opt-in content retention

- J comparison server and both collectors run frozen `270efe156` (v9). Server archive SHA-256: `e7c74eeedc3a58a1152ded9732391cde8d8b6aab7b069779fffbe5030a361963`. Original configuration/database/spools remain in use. At 20:12 UTC, m3 had 51 valid live samples: p95 7.846 seconds, maximum 9.351 seconds, queue p95 1.040 seconds. J had no fresh samples yet. Both had zero queue/rejection/parse/delivery/capacity errors. These short-window samples do not satisfy the 24-hour gate.
- Retention implementation is under verification and has not been applied to either comparison cohort. Nothing runs automatically. Dashboard-authenticated operators review a cutoff, then explicitly remove prompt/response/patch text from at most 25 old turns in inactive sessions. Pinned and task-linked sessions are protected and rechecked under locks. Identities, titles, usage, immutable rates, file counts, notes and native files remain. Summaries covering removed text are cleared.
- A server-owned watermark prevents native/import replay from restoring removed text at the same or older turn time, including newer parser generations. Newer turn activity can supply content again. In-flight summaries are invalidated; a second content check covers retention committing before a summary lease is acquired.
- Surface enumeration: Codex/Claude on m3/m5/J; empty/populated/repeated batches; pinned, linked, recent, archived and unprotected sessions; current/recorded accounting, search, direct/shared history, notes, native/import replay, generation upgrades, resumed turns, summary acquisition/completion races; disabled/unauthorized/invalid API calls; desktop/mobile retention controls and stable keyboard input. Four new PostgreSQL regressions plus existing ingestion/control suites total 23 passing tests; nine route/worker and 11 UI tests pass. Build, lint, gate and browser verification are next.


## Network outage and rollback evidence (20:19–20:21 UTC)

- Only `com.runfusion.agentpulse-comparison-tunnel` was booted out for 300 seconds. Both collectors and all native sessions stayed running. The m3 spool grew to 57 deliveries, including 33 non-coalescing history envelopes. The script restored the same tunnel automatically; afterwards the queue and rejected count were zero, and none of those 33 history event IDs remained pending. Queue removal still requires the exact event ID/body acknowledgement transaction.
- Live lag retained the outage: maximum 296.344 seconds, with a post-restoration sample count of 33 at the first drain check. The combined m3 24-hour sample then had p95 272.643 seconds across 99 updates, explicitly including the outage. J independently measured p95 4.622 seconds across 53 updates. Do not present the outage-inclusive percentile as normal connected latency or erase its samples to pass a target.
- Rollback rehearsal stopped only the m3 comparison collector plus J's two preview units. AgentPulse and its supervisor stayed active and AgentPulse returned HTTP 200 on its existing loopback service. The same preview units/job restarted; all four J service statuses returned active. m3 retained 770 file checkpoints and the exact aggregate revision counter (21,452) across the successful rehearsal. No accepted control was replayed; comparison controls remain disabled.
- The first rollback assertion treated a blank separator in `systemctl` output as an invalid state. Its `finally` restoration succeeded and its original report is preserved. The parser was corrected to ignore separators; the rerun passed with the two explicit inactive states. This was rehearsal-output parsing, not a product failure or lost spool.
- Both real comparison hosts subsequently reported zero queued/rejected deliveries and no parsing or delivery errors. The broader all-host and 24-hour gates remain open. No AgentPulse service was stopped, no production database was changed, and no retention was applied to comparison history.

### Retention validation

Twenty-three PostgreSQL tests, nine route/worker tests, 11 UI tests and three turn validation tests pass. Scoped lint has no errors (existing control-regex/ignored-test warnings only). Serial verification passed all 25 steps in 162.2 seconds; the serial merge gate passed all 754 tests and static validators. Browser inspection checked the review action at 1440px and 390px; mobile content width was 390px, and no remove action was clicked. The final in-flight-summary status message passed all four retention PostgreSQL tests again; its final serial verification passed all 25 steps in 178.7 seconds.

The [reconciliation report](agentpulse-reconciliation.md) now records all seven additional earlier-prompt corrections with their native UUID/time evidence. All seven native messages match the earlier stored owning turns; the remaining later-native rows and unresolved identities are still pending.


## Exact project filter verified

Retention and reconciliation evidence are committed as `6aaffb0df`; another branch push failed for unavailable GitHub HTTPS credentials. The J comparison server remains the verified v9 build `270efe156`, so retention has not been deployed there.

The Sessions list now has a separate exact native project-path filter, composed with host/provider/activity/search/saved filters and retained for pagination. Paths containing spaces are preserved and no title, path-prefix or task inference is performed. Scope covers all hosts/providers, exact/empty/invalid paths, similarly named child paths, page boundaries, keyboard typing and the shared desktop/mobile form. Validation passed: 16 PostgreSQL tests, nine route tests, three UI tests, scoped lint, all 25 serial verification steps (212.8 seconds), and all 754 serial gate tests plus static checks. Desktop/mobile browser checks each returned 50 cards with exactly `/Users/v/dev/scrapeui`; mobile content width was 390px. An initial route mock referenced a hoisted variable too early; that fixture initialization was corrected. m5 still returns `No route to host`.


## Bounded parser-cache maintenance verified (collector v10)

- Added a separate read-only preview and explicit `--apply` maintenance command for old, fully acknowledged native parser caches. It never stops services or runs automatically. Apply requires the same exclusive spool lock as the collector and an empty pending/rejected queue.
- Each invocation inspects at most 50 checkpoints, removes at most five files' parser records, and refuses files above 10,000 records. Native inode/size/mtime must match a complete quiet checkpoint. Missing, partial, changed, recent or oversized files stay protected. A returned keyset cursor continues inspection without a full scan. Native transcripts, live cursors, host binding and durable revision counters remain. SQLite pages become reusable; no blocking VACUUM runs.
...
## Backfill scheduling and source-recorded offline identities

Collector v11 excludes completed unchanged files from the history budget while preserving separate live discovery. Symptom: the m3 cohort still had seven large old Codex files and a resumed Claude file behind because already-completed files consumed bounded visit/read slots. The new planner reduced 772 discovered files to 16 actual work candidates in 16 ms during a read-only inspection. Regression coverage includes 300 completed files ahead of unfinished work, both providers, appends, pending publication, generations, replacement/truncation and retained prefixes. All 59 Python tests pass; native parser generations, limits and delivery checkpoints are unchanged. Deployment is next.

Twelve additional source identities explicitly name m5 and contain matching native SessionStart IDs; one additional zero-turn identity similarly names m3. Their maps record source event IDs, the immutable snapshot hash and `nativeTranscriptVerified: false`. This is historical identity evidence, not live host verification. The preview configuration has a separate hashed m5 ingestion credential; the raw credential is mode 600 and is not in git. All 584 m5 source turns and 12 session records have been acknowledged with zero queued/rejected deliveries. No m5 heartbeat or live collector was invented. The m3 identity-map change is being replayed idempotently to import its additional session record. Twelve remaining hostless source records contain no stored turns and remain unresolved.


## Historical coverage and v11 rollout (21:22 UTC)

All three historical host spools have completed their scans and drained. The additional m3 identity replay preserved newer native data. m5 remains explicitly historical-only. Server authentication returned 401/200 after the c07f8adf3 restart, both collectors report v11, and all four J services (AgentPulse, supervisor, comparison server/collector) stayed active. Original configuration, including the separate m5 credential hash, and all spools were preserved.

Native corroboration and full-snapshot accounting are recorded in the linked reports. The previous 70-identity/5,374-turn cohort is superseded by the 83-identity/5,958-turn audit, without claiming transcript access to offline m5. Four of six source archived sessions are among the unresolved hostless records; their original labels remain in the immutable snapshot. No metadata was assigned to a guessed host.

The new recent-activity overview is read-only and returns at most five sessions, ordered by native activity time rather than import receipt. It uses existing stored Qwen summaries and shows coverage, stale content and failed refreshes; opening it does not schedule agents or inference. Summary freshness now hashes the same bounded input used by the existing summary worker, so changes within a previously summarized turn are visible. Six PostgreSQL tests, 19 UI tests and 11 API/worker tests pass; scoped lint has no errors. Serial build/gate and desktop/mobile inspection are next.


### Recent overview verification complete

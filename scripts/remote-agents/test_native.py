...
class NativeTests(unittest.TestCase):
    ...
    def test_collector_url_restricts_private_http_and_embedded_credentials(self):
        ...
        with patch.dict(os.environ, {'FUSION_REMOTE_AGENTS_HTTP_HOSTS': 'collector-gateway'}):
            self.assertEqual(validated_base_url('http://collector-gateway:4040'), 'http://collector-gateway:4040')
        with self.assertRaises(ValueError):
            validated_base_url('http://collector-gateway:4040')
    ...
    def test_hook_route_update_deduplicates_only_owned_invocations(self):
        ...
        old = shlex.join(['python3', '/owned/feedback_hook.py', '--url', 'http://127.0.0.1:14040', '--project', 'project', '--host', 'workstation', '--provider', 'codex', '--token-file', '/private/token', '--state', '/private/spool'])
        new = old.replace('http://127.0.0.1:14040', 'http://10.8.0.5:4040')

...
6. Add a mandatory \`## Plan Premises\` section containing one to a few atomic, refutable facts that the implementation depends on. Each bullet must be exactly one JSON object using only \`file-exists\`, \`file-absent\`, \`text-present\`, or \`text-absent\`; use a project-relative path and a non-empty literal for text facts. Do not write goals, prose, shell commands, globs, regular expressions, or tautologies as premises.
7. Before persisting PROMPT.md, perform a fresh holistic completeness pass across the whole ledger. Try to disprove the plan by checking referenced paths and patterns, missing consumers, contradictory steps, uncovered failure orderings, and acceptance criteria without tests. Fix all gaps in one pass.
...
3. Validate \`## Plan Premises\`. REVISE when it is absent, empty, malformed, non-atomic, not deterministically verifiable, or expressed as prose/commands/globs/regex. Every bullet must be exactly one JSON object using only \`file-exists\`, \`file-absent\`, \`text-present\`, or \`text-absent\`, with a project-relative path and a non-empty literal for text facts; the facts must be real implementation assumptions rather than goals or duplicated steps.
4. Review at specification altitude. Block when a required behavior, surface, ordering, safety constraint, or proof is missing or the stated approach cannot work. A plan does not need to be internally seamless to be executable — judge whether a competent implementer would build the right thing from it, not whether every section agrees with every other section.
5. If REVISE is necessary, batch **all independently discoverable blocking findings** into this one verdict; do not stop after the first defect. Give each blocker a stable ID, cite the affected section or repository evidence, and state the concrete PROMPT.md correction.
6. After a same-session PROMPT.md edit, verify that the edit resolved what you asked for. Confine that check to the changed sections and their direct dependencies; a fresh whole-artifact re-derivation at this point produces new observations rather than new information.

  ...
  */
  ...
  // NOT EXISTS), so re-running is safe.
    ...
    */
    ...
    if (!applied.includes("0086")) {
      const migrationSql = await readFile(join(MIGRATIONS_DIR, "0086_external_session_native_runtime.sql"), "utf8");
      await tx.execute(sql.raw(migrationSql));
      await tx.execute(sql`INSERT INTO public.${sql.identifier(MIGRATION_BOOKKEEPING_TABLE)} (version) VALUES (${"0086"}) ON CONFLICT (version) DO NOTHING`);
      schemaChanged = true;
    }
    if (!applied.includes("0087")) {
      const migrationSql = await readFile(join(MIGRATIONS_DIR, "0087_external_session_launches.sql"), "utf8");
      await tx.execute(sql.raw(migrationSql));

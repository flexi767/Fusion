...
export const EXTERNAL_SESSION_INGESTION_VERSION = "0079";
...
const EXTERNAL_SESSION_INGESTION_MIGRATION_PATH = join(MIGRATIONS_DIR, "0079_external_session_ingestion.sql");
  ...
  */
  return db.transaction(async (tx) => {
    ...
    */
    ...
    const externalSessionIngestionAlreadyApplied = applied.includes(EXTERNAL_SESSION_INGESTION_VERSION);
  ...
  // NOT EXISTS), so re-running is safe.
    ...
    */
    ...
    if (!externalSessionIngestionAlreadyApplied) {
      const migrationSql = await readFile(EXTERNAL_SESSION_INGESTION_MIGRATION_PATH, "utf8");
      await tx.execute(sql.raw(migrationSql));
      await tx.execute(sql`INSERT INTO public.${sql.identifier(MIGRATION_BOOKKEEPING_TABLE)} (version) VALUES (${EXTERNAL_SESSION_INGESTION_VERSION}) ON CONFLICT (version) DO NOTHING`);
      schemaChanged = true;
    }

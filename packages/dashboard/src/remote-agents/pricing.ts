...
export function priceUsage(value: unknown, provider: string, overrides?: ModelPricingOverrides): RemoteUsage | null {
  ...
  const pricingProvider = provider === "claude_code" ? "anthropic" : provider === "codex_cli" ? "openai-codex" : null;
  const ref = pricingProvider ? { provider: pricingProvider, model: v.model } : null;
  const base = ref ? lookupPricing(ref, overrides) : null;
  ...
  const reason = !ref ? "Provider rate unavailable" : v.fast === true || v.longContext === true ? "Tier rate unavailable" : hour > 0 && rates?.cacheWriteHourPer1M == null ? "One-hour cache rate unavailable" : !rates ? "Model rate unavailable" : null;
  const ordinary = ref ? costFor({ inputTokens: input - cached - write, cachedTokens: cached, cacheWriteTokens: write - hour, outputTokens: output }, ref, undefined, overrides).usd : null;

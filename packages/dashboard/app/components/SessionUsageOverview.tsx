...
export function SessionUsageOverview() {
  ...
  const load = async () => {
    ...
    try { setData(await fetchExternalSessionUsage({ from: `${from}T00:00:00.000Z`, to: `${to}T23:59:59.999Z`, host, model })); }
  ...
  return <details className="session-card"><summary>Session usage and cost rankings</summary>
    ...
    {data && !data.truncated && <>
      <p>{data.from} → {data.to} · {data.sessions.length} sessions with collected turns. Estimates use current configured rates and exclude unpriced groups. External observations are counted separately from Fusion task usage.</p>
      ...
      {data.sessions.slice(0, 50).map((row, index) => <section className="session-card" key={row.id}>
        <h3>{index + 1}. <a href={`?view=sessions&session=${encodeURIComponent(row.id)}`}>{row.title}</a> · {formatCost(row.usd, row.usd === null)}</h3>
        ...
        <SessionCostDetails cost={{ usd: row.usd, usage: row.usage, coveredTurns: row.turns, unreportedTurns: row.unreportedTurns, unpricedRows: row.unpricedRows }} label="Cost breakdown for selected turns" />
      </section>)}
      {data.sessions.length > 50 && <p>Showing the 50 most expensive sessions of {data.sessions.length} matched sessions.</p>}
    </>}
  </details>;
}

...
export function SessionUsageOverview() {
  ...
  const load = async () => {
    ...
    try { setData(await fetchExternalSessionUsage({ from: `${from}T00:00:00.000Z`, to: `${to}T23:59:59.999Z`, host, model, groupBy })); }
  ...
  return <details className="session-card"><summary>Session usage and cost rankings</summary>
    ...
    {data && !data.truncated && <>
      <p>{data.from} → {data.to} · {data.sessions.length} ranked results with collected turns. Estimates use current configured rates and exclude unpriced groups. External observations are counted separately from Fusion task usage.</p>
